...
export class WeatherapiService {
  ...
  async getCurrentLocation() {
    ...
    this.http.get(`${environment.apiUrl}/locations/v1/cities/geoposition/search?apikey=${environment.apiKey2}&q=${latitude}%2C${longitude}`).subscribe(
      (data: any) => {
      ...
      async (error) => {
        ...
        const cachedHourlyData = await this.sqliteService.getHourlyWeatherData(
          `${latitude},${longitude}`
        );
        ...
        if (cachedHourlyData) {
          console.log('Using cached hourly weather data:', cachedHourlyData);
  ...
  async getLocationDetails(locKey: string): Promise<void> {
    try {
      const data: any = await firstValueFrom(
        this.http.get(`${environment.apiUrl}/locations/v1/${locKey}?apikey=${environment.apiKey2}`)
      );
  
      this.weatherData.englishName = data.EnglishName;
      this.weatherData.adminName = data.AdministrativeArea.EnglishName;
  
      await this.getWeatherConditions(locKey);
      await this.getForecast5Days(locKey);
      await this.getHourlyUpdates(locKey);
  
      await this.sqliteService.saveWeatherData(
        this.weatherData.englishName,
        JSON.stringify(this.weatherData)
      );
      await this.sqliteService.saveHourlyWeatherData(
        this.weatherData.englishName,
        JSON.stringify(this.hourlyWeatherData)
      );
      ...
      console.error('Error fetching location details or saving to SQLite:', error);
    }
  }

  private async getWeatherConditions(locKey: string): Promise<void> {
    try {
      const data: any = await firstValueFrom(
        this.http.get(`${environment.apiUrl}/currentconditions/v1/${locKey}?apikey=${environment.apiKey2}&details=true`)
      );
  ...
  private async getForecast5Days(locKey: string): Promise<void> {
    try {
      const data: any = await firstValueFrom(
        this.http.get(`${environment.apiUrl}/forecasts/v1/daily/5day/${locKey}?apikey=${environment.apiKey2}&metric=true`)
      );
  ...
  private async getHourlyUpdates(locKey: string): Promise<void> {
    try {
      const data: any = await firstValueFrom(
        this.http.get(`${environment.apiUrl}/forecasts/v1/hourly/12hour/${locKey}?apikey=${environment.apiKey2}&metric=true`)
      );
  ...
  getAutoCompleteLocations(q: string) {
    ...
    this.http.get(`${environment.apiUrl}/locations/v1/cities/autocomplete?apikey=${environment.apiKey2}&q=${q}`).subscribe(
      (data: any) => {
